...
\n (2) a train simulator, which you will use to develop and test your the final project.
*/
...
/*! 
\struct WINDOW
\brief Struct that holds properties of a specific window.
...
As a result, this variable kernel_window is a pointer to that WINDOW.
*/
/* TOS_ENDIF never */
...
\details Processes in TOS can be created, but are never deleted.  There is a PCB entry for every TOS process.  These entries are never deleted; once a process's PCB entry is created, it is never removed.  After a TOS process has accomplished its task, the state of the TOS process can be changed to #STATE_ZOMBIE to indicate that it will not run again.  
*/
/* TOS_ENDIF never */
#define MAX_PROCS		20
...
\details Only processes with this state can be on the #ready_queue.  If a process is in any other state, it is not eligible for CPU cycles and must be off the ready queue.  
*/
/* TOS_ENDIF never */
...
/* TOS_IFDEF never */
/*!
\brief This state is assigned to processes that are permanently off the ready queue and will never run again.
...
\details When the sender uses send() instead of message(), it is because the sender needs a reply from the receiver before the sender can be put back on the ready queue.  This state is used to indicate that the sender is awaiting a reply from the receiver process.  After the receiver executes reply(), the sender process goes from this state to #STATE_READY and is put back on the ready queue.
*/
/* TOS_ENDIF never */
...
\details In inter-process communication, if a TOS process calls receive() but no other process has sent it a message (either via send() or message()), then the process that called receive() enters this blocked state in order to await the sending of a message by another process.  Once another process sends a message via either send() or message() to this process while this process is receive-blocked, then this process goes from receive-blocked to #STATE_READY.
*/
/* TOS_ENDIF never */
...
\details In inter-process communication, if a TOS process calls message() but the receiver process has not yet called receive(), it means that the receiver is not ready to receive the message, and the sender process must wait until the receiver calls receive().  Until the receiver calls receive(), the sender process waits in this blocked state.  Unlike #STATE_SEND_BLOCKED, a process in this state does not expect a reply from the receiver, so that when the receiver calls receive(), a #STATE_MESSAGE_BLOCKED process can be changed to #STATE_READY and be put back on the ready queue.   
*/
/* TOS_ENDIF never */
...
\details TOS utilizes keyboard interrupts, timer interrupts, and COM1 interrupts to accomplish various functions.  A TOS process that is waiting for any of these interrupts is placed into this state and remains off the ready queue.  When the interrupt occurs, the process is changed to #STATE_READY and put back on the ready queue so that it can resume activity.
*/
/* TOS_ENDIF never */
...
/* TOS_IFDEF never */
/*!
\brief A value used to verify that a data structure is a PCB entry.
\details "MAGIC" codes are used in TOS as a debugging and security feature in assert() statements.  PCB entries store this value in a field.  When a process attempts to access a PCB entry, this field is used to verify that the data structure being accessed is in fact a PCB entry.  Failure of this condition stops execution of TOS.
*/
/* TOS_ENDIF never */
...
/*!
\typedef PROCESS
\brief Pointer to a _PCB struct, also known as a PCB.
\details The pcb[] array holds the PCB entry for each process.  
This typedef allows PROCESS to be used as an alternative name for a pointer to a PCB in the pcb[] array.
\sa _PCB, PCB, pcb
*/
/* TOS_ENDIF never */
...
/*!
\struct _PORT_DEF
\brief A data structure that holds the members associated with a TOS process's port, used for inter-process communication.
\details The underscore in the struct's name indicates that it will not be necessary to refer directly by name to this data structure
when implementing TOS.  Rather, the typedef PORT can be used to access this struct more conveniently.
...
<TABLE>
  ...
  <TR>  
    <td>unsigned used</td>
    <td>An unsigned integer indicating whether the port slot in the port[] array is being used or not.</td>
  </TR>
  <TR>  
    <td>unsigned open</td>
  ...
  <TR>  
    <td>PROCESS blocked_list_head</td>
    <td>The first process on the send-blocked list of this port.</td>
  </TR>
  <TR>  
    <td>PROCESS blocked_list_tail</td>
...
/*! 
\struct _PCB
\brief Struct that holds the information associated with a TOS process.
\details This struct is used to store information about a TOS process.  Each entry in the pcb[] array is one of these structs.
...
<TABLE>
  ...
  <TR>
    <td>unsigned used</td>  
    <td>An integer value indicating whether this PCB entry in the pcb[] array has been assigned to a process yet.
        Once assigned, this PCB entry is never freed up or re-assigned to a different process.  
    </td>
  </TR>
  <TR>
    <td>unsigned short priority</td> 
    <td>An integer from 0 to 7 corresponding to the priority leve of this process.</td>
  </TR>
  <TR>
  ...
  <TR>
    <td>PROCESS param_proc</td>  
    <td>A temporary storage location for the PROCESS that was passed as an input to send() or message().</td>
  </TR>
  <TR>
...
The array pcb[] holds 20 of these PCB structs.  
The typedef PCB is created to serve as another name for this struct.  
The underscore in the name _PCB indicates that it will not be necessary to refer directly to the PCB struct when implementing the TOS API.
\sa PCB, PROCESS
*/
/* TOS_ENDIF never */
typedef struct _PCB {
...
/* TOS_IFDEF never */
/*!
\brief An array of twenty PCB structs.
\details Each element in this array corresponds to the PCB entry for one TOS process.
TOS processes are never removed from this array: when a process has completed its task, 
its state becomes #STATE_ZOMBIE and it remains in the PCB array.
\sa create_process(), _PCB
*/
/* TOS_ENDIF never */
extern PCB pcb[];
...
/* TOS_IFDEF never */
/*! 
\brief Creates a new TOS process and returns the PORT owned by the new process (or, before PORTs are covered, returns a NULL pointer).
\details When the running process calls this function, a new TOS process is created through the following steps:
(1) The next available PCB entry in the pcb[] array is allocated;   
...
(7) return either a null pointer (before the class covers ports) or the PORT of the new process.
...
\return PORT The port owned by the newly-created process.   
\sa PORT, PARAM, PROCESS, PCB, pcb, _PORT_DEF, PORT_DEF
*/
/* TOS_ENDIF never */
...
/* TOS_IFDEF never */
/*!
\brief Creates and returns a copy of the calling process
\details This function initializes a new TOS process to be an exact copy of the calling process.  The program code, heap, and stack
are all copied.  The new process occupies a different PCB entry and has a different stack frame 
than the calling process and is in fact a distinct process from the calling process.
\sa create_process(), PCB, pcb
*/
/* TOS_ENDIF never */
#ifdef XXX
PROCESS fork();
...
One possible implementation of this function calls print_process() for each of the processes in the PCB array. 
\param[in] wnd Pointer to the WINDOW to which the process details should be printed
\sa print_process(), pcb, PCB
*/
/* TOS_ENDIF never */
...
\details This function clears all the entries in the pcb[] array; prepares the doubly-linked list of entries so that each pcb entry contains the address of the next available PCB slot; prepares the pcb entry for the boot process, and sets the global variable active_proc (used throughout TOS) to the first entry of the pcb array.
\sa pcb, PCB, PROCESS
*/
/* TOS_ENDIF never */
void init_process();
...
\sa ready_queue, PCB, add_ready_queue()
*/
/* TOS_ENDIF never */
...
/* TOS_IFDEF never */
/*!
\brief Specifies the maximum number of PORTs supported in TOS.
\details In the inter-process communication of TOS, messages are delivered to PORTs.  Each TOS process can have one or more PORTs, but each PORT can be owned by exactly one process.  This number is the size of the array that holds the PORT entries.
*/
/* TOS_ENDIF never */
...
/* TOS_IFDEF never */
/*!
\brief A value used to verify that a data structure is a PORT.
\details "MAGIC" codes are used in TOS as a debugging and security feature in assert() statements.  Each PORT data structure entries store this value in its "magic" field.  When a process attempts to access a PORT, this field is used to verify that the data structure being accessed is in fact a PORT.  Failure of this condition stops execution of TOS.
*/
/* TOS_ENDIF never */
...
\sa PORT, PROCESS, active_proc, create_new_port()
*/
/* TOS_ENDIF never */
PORT create_port();
...
\details This function creates and returns a new port for the PROCESS indicated by proc.  The port is initially open. Note that while a process can own multiple ports, each port can be owned by exactly one process.
\sa PORT, PROCESS, active_proc, create_port()
*/
/* TOS_ENDIF never */
...
/*!
\brief Opens a port.
\details Opens the PORT indicated by the parameter port.  Only messages that are in an open port can be delivered to the receiver.  (Messages can still be sent to a closed port, but they will not be delivered to the receiver; instead, the messages are queued for later delivery.)
\sa close_port()
*/
/* TOS_ENDIF never */
...
/*!
\brief Closes a port.
\details Closes the PORT indicated by the parameter port.  Messages can still be sent to a closed port, but they will not be delivered to the receiver.  Instead, they are queued for delivery at some later time, after the port has been re-opened.  
\sa open_port()
*/
/* TOS_ENDIF never */
...
/* TOS_IFDEF never */
/*!
\brief Sends a message to the PORT of a receiver process, from which no reply is expected.
...
\param[in] dest_port A PORT owned by the receiver process, which must be open if the message is to be deliverable.  See the slides on inter-process communication for more details.  
\param[in] data A void pointer to the data contained in the message.
\sa PORT, send(), receive(), reply()
*/
/* TOS_ENDIF never */
...
\param[out] sender A pointer which is modified by the receive() function to point to the sender PROCESS (as such, this is an output argument).
\return A void pointer passed by the sender process to the content of the message.     
\sa PORT, send(), message()
*/
/* TOS_ENDIF never */
...
\param[in] sender The PROCESS which sent the message and is awaiting the reply.
...
\details This function is called by kernel_main() to initialize the entries of the array of PORTs held by the global variable port[].  Among other things, each element of the array contains a pointer to the next element of the array and a field indicating whether the array entry is being used by a process. See the "IPC" slide deck for more information about these global variables.
\sa PORT, PORT_DEF
*/
/* TOS_ENDIF never */
void init_ipc();
...
/* TOS_IFDEF never */
/*!
\brief PORT owned by the timer process.
...
\sa Timer_Message, _Timer_Message, sleep()
*/
/* TOS_ENDIF never */
...
\details This function is a wrapper that client processes use to create and send a Timer_Message to the timer process, indicating that the client process wishes to sleep for the number of timer ticks indicated by the argument num_of_ticks.  This integer is contained in the Timer_Message that is generated and sent to the timer process.
...
\details It is important to note that these I/O ports are not to be confused with the PORTs used by TOS processes for inter-process communication.
*/
/* TOS_ENDIF never */
...
/* TOS_IFDEF never */
/*!
\brief One of the PORTs owned by the COM process to receive messages from user processes.
\details This global variable refers to the PORT which is owned by the COM process and receives messages (of type COM_Message) from user processes that wish to send and/or receive bytes from the COM1 I/O port.  This global is initialized in init_com(). Note that the COM process owns a second port, at which messages from the COM Reader process are received.  See the "Device Drivers" lecture slides for more information about the COM Process and COM Reader Process.
\sa init_com(), COM_Message, _COM_Message
*/
/* TOS_ENDIF never */
...
/* TOS_IFDEF never */
/*!
\brief The PORT owned by the keyboard process.
...
\details This process creates the keyboard process and assigns the PORT returned by the call to create_process() to the global variable keyb_port.  Note: this process must be called after the initialization of the Window Manager via init_wm(). 
...
/* TOS_IFDEF never */
// Files must be commented in order to be detectable with EXTRACT_ALL = NO specified in doxyfile.
...
You compile TOS using either "make" or "make tests".  The latter will build a testing kernel, and when TOS runs, the kernel_main() function in the test directory, rather than this kernel_main(), will execute.   
*/
/*! \file kernel/null.c */
/*! \file kernel/pong.c */
